...
public:
    ...
    */
    //Hashing qgram : Abbildung aller q-gramme auf Positionen in Dir
...
private:
   ...
   std::vector<uint32_t> hashW;
   void Countingsort (const std::string& text, std::vector<uint32_t>& dir, std::vector<uint32_t>& Sa,uint8_t qrsize,uint32_t txsize);
};
